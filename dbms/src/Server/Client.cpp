...
#undef EXTRACT_LIMIT
    ...
    // Some temporary auxiliary functions to help me write the macro engine
    ...
    template <template<class...>class Container = std::vector, class String> Container<String>
    split(const String& to_split, const dont_deduce_t<String>& delimiter) {
    ...
    String replace(const String& original, const dont_deduce_t<String>& pattern, const dont_deduce_t<String>& replacement) {
        auto container = split(original, pattern);
        return join_container(container, replacement);
    }
    // Some temporary auxiliary functions to help me write the macro engine


    bool process(const String & query)
    {
        // Macro engine begin
        constexpr auto macro_token{"#"};
        constexpr auto create_macro{"$#"};
        constexpr auto body_being_token{"="};
        constexpr auto parts_separator{"$"};
        constexpr auto variable_marker{"$"};

        static std::unordered_map<std::string, std::string> macro_dictionary{};
       std::string query_mine = query;
        if(query_mine.find(create_macro) == 0) {
            auto mid = query_mine.find(body_being_token);
            auto macro_name = query_mine.substr(2, mid - 2);
            auto macro_body = query_mine.substr(mid + 1, query_mine.size());
            macro_dictionary.insert({macro_name, macro_body});
            return true;
        }

        std::vector<std::pair<size_t, size_t>> macro_positions{};
        std::pair<size_t, size_t> add{-1,-1};
        auto old_pos = -1;
        while(true) {
            auto token_position = query_mine.find(macro_token, old_pos + 1);
            if(token_position == std::string::npos)
                break;
            old_pos = token_position;
            if(add.first == -1) {
                add.first = token_position;
                continue;
            }
            if(add.second == -1) {
                add.second = token_position;
                macro_positions.push_back(add);
                add = {-1,-1};
            }
        }
        std::string new_query{""};
        auto lp = 0;
        for(const auto& position : macro_positions) {
            auto macro_text = query_mine.substr(position.first + 1, position.second - position.first - 1);
            const auto parts = split(macro_text, parts_separator);
            auto macro_content = macro_dictionary.find(parts[0])->second;
            std::cout << "Macro text: " << macro_content << std::endl;
            for(auto i = 1; i < parts.size(); i++) {
                std::string token = variable_marker + std::to_string(i);
                macro_content = replace(macro_content, token, parts[i]);
            }
            new_query += query_mine.substr(lp, position.first - lp);
            std::cout << "Substrang between: " << lp << "  " << position.first << std::endl;
            std::cout << "Query is now: " << new_query << std::endl;
            new_query += macro_content;
            std::cout << "Query is now: " << new_query << std::endl;
            lp = position.second + 1;
        }
        new_query += query_mine.substr(lp, query_mine.size());
        std::cout << "\n\n\n" << query_mine << "\n\n\n" << std::endl;
        const String line = new_query.c_str();
        // :> Macro engine end
